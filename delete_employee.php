...
if (isset($_GET['id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        // If the form is submitted, delete the employee record
        $query = "DELETE FROM employee WHERE employee_id = '$id'";
        if (mysqli_query($conn, $query)) {
            // Redirect back to the dashboard with a success message
            header('Location: ../admin.php?message=Employee deleted successfully');
            exit;
        } else {
            // Redirect back with an error message
            header('Location: ../admin.php?error=Error deleting employee: ' . mysqli_error($conn));
            exit;
        }
    } else {
        ...
        <body>
            <div class='container mt-5'>
                ...
                <form method='POST' action='delete_employee.php?id=$id'>
